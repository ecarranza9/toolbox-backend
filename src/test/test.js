    ...
    it('it should GET a valid files data', (done) => {
      chai.request(app)
        ...
        .end((err, res) => {
          ...
          expect(res.body[0][0]).to.have.property('lines').to.be.an('array')
          done()
        })
    })
    ...
    it('it should GET a specific file data and this is valid', (done) => {
      chai.request(app)
        ...
        .end((err, res) => {
          ...
          res.should.have.status(200)
          res.body.should.be.a('array')
          expect(res.body[0][1]).to.have.property('file')
          expect(res.body[0][1]).to.have.property('lines')
          expect(res.body[0][1].lines[0]).to.have.property('text').to.be.a('string')
          expect(res.body[0][1].lines[0]).to.have.property('number').to.be.a('number')
          expect(res.body[0][1].lines[0]).to.have.property('hex').to.be.a('string').lengthOf(32)
          done()
        })
    })
  })
